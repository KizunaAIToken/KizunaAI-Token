 /**
 * Welcome to KAT
 * KizunaAI Token ($KAT) BSC's first music animation GAME and NFTs
 *
 *                                                                                                                               
...
contract KizunaAIToken is ERC20, Ownable, Pausable {
    // CFG HEAD
    
    uint256 private initialSupply = 500_000_000_000_0 * (10**6);
   
    uint256 private denominator = 1000;

    uint256 private swapThreshold = 0.0000005 ether;
    
    uint256 private devTaxBuy = 50;
    uint256 private marketingTaxBuy = 50;
    uint256 private charityTaxBuy = 0;
    uint256 private backbuyburnTaxBuy = 0;
    
    uint256 private devTaxSell = 50;
    uint256 private marketingTaxSell = 50;
    uint256 private charityTaxSell = 0;
    uint256 private backbuyburnTaxSell = 0;
    
    address private devTaxWallet = 0x129EC7acb2aD9EABfC05ddf69c0319AcD526eF2c;
    address private marketingTaxWallet = 0x8Ec4708B7f46376d88d35B1A7A67EdDA6EC45F35;
    address private charityTaxWallet = 0x1062499d81941D449Bdb56587F6D656C34e42f16;
    address private backbuyburnTaxWallet = 0x129EC7acb2aD9EABfC05ddf69c0319AcD526eF2c;
    ...
    constructor() ERC20("KizunaAI Token", "KAT")
    {
    ...
    function _transfer(
        ...
        require(!paused(), "Kizuna AI Token: token transfer while paused");
        require(!isBlacklisted(msg.sender), "Kizuna AI Token: sender blacklisted");
        require(!isBlacklisted(recipient), "Kizuna AI Token: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "Kizuna AI Token: sender blacklisted");
    ...
    function pause() public onlyOwner {
        require(!paused(), "Kizuna AI Token: Contract is already paused");
        _pause();
    }

    function unpause() public onlyOwner {
        require(paused(), "Kizuna AI Token: Contract is not paused");
        _unpause();
    }
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "Kizuna AI Token: Account is already blacklisted");
        blacklist[account] = true;
    }
    
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "Kizuna AI Token: Account is not blacklisted");
        blacklist[account] = false;
    }
    
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "Kizuna AI Token: Account is already excluded");
        excludeList[account] = true;
    }
    
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "Kizuna AI Token: Account is not excluded");
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "Kizuna AI Token: Tax is already enabled");
        taxStatus = true;
    }

    function disableTax() public onlyOwner {
        require(taxStatus, "Kizuna AI Token: Tax is already disabled");
        taxStatus = false;
    }
